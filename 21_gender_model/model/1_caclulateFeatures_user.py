...
#------------------------------------------------------------------------------
# GENDEL CLASSIFYING MODEL - BUILD MODEL - EXTRACTING FEATURES
#------------------------------------------------------------------------------
...
# Read lexiconns data
ageLex_file = '.\\lexicons\\emnlp14gender.csv'
...
# Function to calcualte gender score of a tweets using Gender Lexicon
...
def add_featuresMsg(row):
    ...
    row['nEmoticons'] = f_emoticons(line) #len(f_emoticons(line))
...
def tokenize_line(text):
    ...
    # Eliminate user
    user_regex = '(@[A-Za-z\_]+[A-Za-z0-9_]+)' #'(?<=^|(?<=[^a-zA-Z0-9-_\.]))@([A-Za-z\_]+[A-Za-z0-9_]+)'
...
def parseTwitterFile(fileNameIn, maxTweet=-1):
    ...
    userName = extractUserName_fromFileName(fileNameIn)     
    ...
    dataIn['wordLength'] = 0
    #dataIn['nPronouns'] = 0 # Took to long, remove
    dataIn['nSingularP'] = 0
    ...
    #dataIn['nPronouns'].update(tokens_series.apply(nltk.pos_tag).apply(f_numberPronouns))#pos tag took too long
    
    #t0 = time()
    dataIn['nSingularP'].update(tokens_series.apply(f_nSingularP))
    #print(time()-t0)

    # Find features for this user (avg between mesages and lecicon score)
    #t0 = time()
    ...
    #t0 = time()
    feature_v = [userName, str(userLexiconScore), str(avg_isRT),
...
if __name__ == '__main__':
    ...
    features_names = ['userScreen_name', 'userLexiconScore', 'avg_isRT',
    ...
    inputDir = '.\\input\\user_tweetsGender\\' # Input dir
    fileNameOut = '.\\output\\allUsers_tweets_featuresGender.tsv' # Output file 
    
    t0 = time()
    n_records = 0
    with open(fileNameOut, 'w') as fileOut:
        
        # Write the columns names to the file
